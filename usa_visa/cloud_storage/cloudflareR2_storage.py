...
from usa_visa.configuration.cloudflare_connection import R2Client
...
class SimpleStorageService:
    ...
    def __init__(self):
        r2_client = R2Client()
        self.s3_resource = r2_client.r2_resource
        self.s3_client = r2_client.r2_client
    ...
    def upload_df_as_parquet(self, data_frame: DataFrame, local_filename: str, bucket_filename: str, bucket_name: str) -> None:
        """
        Method Name :   upload_df_as_parquet
        Description :   This method uploads the dataframe to bucket_filename parquet file in bucket_name bucket

        Output      :   Folder is created in s3 bucket
        On Failure  :   Write an exception log and then raise an exception
        """
        
        logging.info("Entered the upload_df_as_parquet method of S3Operations class")

        try:
            data_frame.to_parquet(local_filename, index=None, header=True)

            self.upload_file(local_filename, bucket_filename, bucket_name)

            logging.info("Exited the upload_df_as_parquet method of S3Operations class")
    ...
    def get_df_from_object(self, object_: object) -> DataFrame:
        ...
        try:
            ...
            df = read_parquet(content, na_values="na")
            logging.info("Exited the get_df_from_object method of S3Operations class")
            return df
        except Exception as e:
            raise AydieException(e, sys) from e

    def read_parquet(self, filename: str, bucket_name: str) -> DataFrame:
        """
        Method Name :   get_df_from_object
        Description :   This method gets the dataframe from the object_name object
        ...
        logging.info("Entered the read_parquet method of S3Operations class")

        try:
            parquet_obj = self.get_file_object(filename, bucket_name)
            df = self.get_df_from_object(parquet_obj)
            logging.info("Exited the read_parquet method of S3Operations class")
            return df
        except Exception as e:
            raise AydieException(e, sys) from e
